...
import {useDisclosure, Input, Button, Drawer, DrawerBody, DrawerFooter, DrawerHeader, DrawerOverlay, DrawerContent, DrawerCloseButton} from '@chakra-ui/react'

const ReviewContainer = () => {
  const [isUserSignedIn, setUserSignedIn] = useState(false);
  const [showWriteReview, setShowWriteReview] = useState(false);
  const [bookmarkBlack, setBookmarkBlack] = useState(false);
  ...
  const toggle = () => {
    if (checkUserSignIn()) {
      setBookmarkBlack(!bookmarkBlack);
    } else {
      redirectToLoginPage();
    }
  };
  ...
  return (
    <div>
      <section className="review-container" style={{ maxHeight: '743px', overflowY: 'auto' }}>
      <div className="title-and-bookmark">
        <div className="bit-title">
          <h2 id="title">Externship Title</h2>
        </div>
        <button id="bookmark" onClick={toggle}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="19"
            height="26"
            viewBox="0 0 19 26"
            fill={bookmarkBlack ? "black" : "white"}
          >
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
    ...
    /*<>
      ...
      >
        ...
        <DrawerContent>
          <DrawerCloseButton />
          <DrawerHeader>
            <h1 id="title">Externship Title</h1>
          </DrawerHeader>
          <button id="bookmark" onClick={toggle}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
